...
async function refreshRepoModel(config, repo) {
  ...
  repo.openPRs = prMap.tool[repo.name] || [];
  repo.pending = repo.openPRs.length > 0;
  repo.dependabotPRs = prMap.dependabot[repo.name] || null;
  if (repo.openPRs.length) {
    const { suggested, candidates } = await fetchReviewerOptions(config.org, `${config.org}/${repo.name}`);
    repo.reviewerOptions = candidates;
    repo.suggestedReviewer = suggested;
  }
...
async function buildModel(config) {
  ...
  for (const r of list) {
    r.openPRs = prMap.tool[r.name] || [];
    r.pending = r.openPRs.length > 0;
    r.dependabotPRs = prMap.dependabot[r.name] || null;
  }
  ...
  // You can't review your own PR, and every tool PR is authored by the authed user — so
  // leaving them in the picker would offer a choice GitHub rejects.
  let me = null;
  try { me = await whoami(); } catch { /* health endpoint reports auth trouble */ }
  const notMe = (x) => !me || x.handle !== me;
  await mapLimit(list.filter((r) => r.openPRs.length), 8, async (r) => {
    const { suggested, candidates } = await fetchReviewerOptions(org, `${org}/${r.name}`);
    r.reviewerOptions = candidates.filter(notMe);
    r.suggestedReviewer = notMe(suggested || {}) ? suggested : r.reviewerOptions[0] || null;
  });
  // Fallback roster for repos whose history has nobody to suggest. Model-level rather than
  // copied onto each repo — it's the same list for all of them.
  const orgMembers = (await fetchOrgMembers(org)).filter(notMe);
  ...
  return { org, generatedAt: new Date().toISOString(), totals, repos: list, orgMembers };
}
...
async function enrichComplianceRepos(org, repos) {
  await mapLimit(repos, 16, async (r) => {
    ...
    r.dependsOnOrg = [...refs];
    r.gitDeps = [...gitDeps];
    // Dependabot liveness: what the repo ASKS for, and when each ecosystem last actually
    // ran. Runs are only fetched when there's a config to compare them against — a repo
    // with no dependabot.yml can't have a stale ecosystem, so it costs nothing there.
    r.dependabotConfig = dependabot.parseDependabotConfig(await ghFileRaw(org, r.name, ".github/dependabot.yml"));
    r.dependabotRuns = r.dependabotConfig.length ? await dependabot.fetchRuns(org, r.name) : [];
...
/**
 * Flag, on each of OUR open PRs, the foreign PRs it will collide with on merge: same base
 * branch, same lockfile. Two PRs that both regenerate `Gemfile.lock` from the same base can't
 * both merge cleanly — the second one has to be rebased and its lockfile rebuilt.
 *
 * The consolidation cluster deliberately only ever considers our own PRs, because every action
 * it offers (rollup CLOSES the originals, stack REWRITES their branches, sequence comments on
 * them) must never reach a PR this tool didn't open. That safety rule left exactly this case
 * unreported: a human's PR racing ours for the same lockfile was invisible. Reporting it is
 * safe; acting on it is not — so this only annotates, and nothing downstream consolidates it.
 *
 * DEPENDABOT'S OWN PRs ARE EXCLUDED, and not merely to cut noise. They collide with our
 * consolidated PR *by construction* — superseding them is the point, which is what
 * `nudgeDependabotOnClear` retires them for — and Dependabot rebases its own PR when the base
 * moves. So "one of you has to rebase", the whole message of this warning, is never true of
 * them. Measured on headwayio/crows-nest: 15 collisions, 13 of them Dependabot. Reporting
 * those would fire on every repo forever and bury the two human PRs that actually needed
 * a conversation.
 */
const isDependabotBranch = (head) => String(head || "").startsWith("dependabot/");

/**
 * Summarize the open Dependabot VERSION-update PRs on a repo — the routine bumps this tool
 * deliberately never acts on. Not acting on them is right (auto-merging arbitrary majors
 * would dilute the audit trail and risk breakage); leaving them invisible is not, because
 * today the only way to notice is to browse each repo's PR list by hand. They matter for
 * two reasons: falling far behind is how a future security patch ends up unappliable, and
 * enough of them open at once silently trips `open-pull-requests-limit`, which stops
 * Dependabot running that ecosystem at all (see lib/dependabot.js).
 *
 * Read-only by construction — this returns counts and links, and nothing downstream may act
 * on them. `dependabot/` never matches isToolBranch, so they can't be rolled up or closed.
 *
 * Ecosystem comes from the branch (`dependabot/<eco>/<pkg>-<version>`); `github_actions` is
 * CI plumbing and is counted apart from application dependencies, which are the ones that
 * rot. Bump type is parsed from Dependabot's own title, reusing bumpType().
 */
function summarizeDependabotPRs(foreign) {
  const prs = [];
  for (const p of foreign || []) {
    if (!isDependabotBranch(p.headRefName)) continue;
    const eco = String(p.headRefName).split("/")[1] || "unknown";
    const m = /Bump (\S+) from (\S+) to (\S+)/i.exec(p.title || "");
    const checks = (p.statusCheckRollup || []).map((c) => String(c.conclusion || c.state || "").toUpperCase());
    prs.push({
      number: p.number,
      url: p.url,
      title: p.title || "",
      ...
      from: m ? m[2] : null,
      to: m ? m[3] : null,
      bump: m ? bumpType(m[2], m[3]) : "",
      createdAt: p.createdAt || null,
      // Only a definite FAILURE counts. A PR still running, or one with no checks at all,
      // isn't a problem to report — flagging those would make the number meaningless.
      failing: checks.includes("FAILURE") || checks.includes("ERROR") || checks.includes("TIMED_OUT"),
    });
  }
  if (!prs.length) return null;
  const infra = prs.filter((p) => p.ecosystem === "github_actions");
  const app = prs.filter((p) => p.ecosystem !== "github_actions");
  const byEcosystem = {};
  for (const p of prs) byEcosystem[p.ecosystem] = (byEcosystem[p.ecosystem] || 0) + 1;
  const oldest = prs.map((p) => Date.parse(p.createdAt || 0) || 0).filter(Boolean).sort()[0] || null;
  return {
    total: prs.length,
    app: app.length,
    infra: infra.length,
    major: prs.filter((p) => p.bump === "major").length,
    failing: prs.filter((p) => p.failing).length,
    oldestAt: oldest ? new Date(oldest).toISOString() : null,
    byEcosystem,
    prs: prs.sort((a, b) => (Date.parse(a.createdAt || 0) || 0) - (Date.parse(b.createdAt || 0) || 0)),
  };
}

function annotateForeignCollisions(ours, foreign) {
  const theirs = foreign
    .filter((p) => !isDependabotBranch(p.headRefName))
    .map((p) => ({ pr: p, locks: prLockfiles(p.files) }))
    .filter((x) => x.locks.length);
  if (!theirs.length) return;
  for (const mine of ours) {
    const hits = [];
    for (const { pr, locks } of theirs) {
      if ((pr.baseRefName || null) !== mine.baseRefName) continue; // different base → no direct clash
      const shared = locks.filter((l) => mine.lockfiles.includes(l));
      ...
      hits.push({
        ...
        title: pr.title || "",
        draft: !!pr.isDraft,
        author: (pr.author && (pr.author.login || pr.author.name)) || null,
        lockfiles: shared,
      });
    }
    if (hits.length) mine.collidesWith = hits;
  }
}
...
async function fetchToolPRs(org, names, branchPrefix) {
  ...
  await mapLimit(names || [], 8, async (name) => {
    const prs = (await ghJson([
      ...
      // statusCheckRollup is only here for the Dependabot summary's failing count. It is the
      // one field that meaningfully grows the response, so it earns its place by being the
      // difference between "12 open PRs" (trivia) and "9 of them are red" (a maintenance signal).
      "--json", "number,url,isDraft,createdAt,headRefName,baseRefName,title,reviewDecision,reviewRequests,files,author,statusCheckRollup",
      "--limit", "50",
    ])) || [];
    // Split rather than filter. The PRs we didn't open were being thrown away here, which is
    // why nothing downstream could see a human's PR racing ours for the same lockfile — and
    // this call already has them in hand, so noticing costs no extra request.
    const ours = [];
    const foreign = [];
    for (const pr of prs) {
      if (!isToolBranch(pr.headRefName, branchPrefix)) { foreign.push(pr); continue; }
      ours.push({
        number: pr.number,
        url: pr.url,
        draft: pr.isDraft,
    ...
    // Summarize BEFORE the no-tool-PRs bail-out: a repo with nothing of ours open is
    // exactly where a Dependabot backlog piles up unnoticed.
    const db = summarizeDependabotPRs(foreign);
    if (db) dependabotMap[name] = db;
    if (!ours.length) return;
    annotateForeignCollisions(ours, foreign);
    map[name] = ours;
  });
  return { tool: map, dependabot: dependabotMap };
}
 ...
 */
async function fetchReviewerOptions(org, nwo) {
  const prs = (await ghJson([
  ...
  const byHandle = new Map(); // insertion order IS the recency order the UI shows
  const add = (handle, display, isTeam) => {
    if (handle && !byHandle.has(handle)) byHandle.set(handle, { handle, display, isTeam });
  };
  for (const pr of prs) {
    for (const x of pr.reviewRequests || []) {
      if (x.login) add(x.login, x.login, false);
      else if (x.slug) add(`${org}/${x.slug}`, x.slug, true);
    }
    // Then whoever actually reviewed it, newest review first — so a PR with no pending
    // request still contributes its most recent reviewer ahead of older PRs' people.
    for (const rv of [...(pr.reviews || [])].reverse()) {
      const login = rv.author && rv.author.login;
      if (login) add(login, login, false);
    }
  }
...
// Org roster, for the "nobody has ever reviewed this repo" case — a brand-new repo has no
// history to suggest from, so the picker offers everyone rather than nothing. Cached for
// an hour: membership changes far more slowly than a Refresh cycle.
let orgMemberCache = null;
let orgMemberAt = 0;
async function fetchOrgMembers(org) {
  if (orgMemberCache && Date.now() - orgMemberAt < 3600000) return orgMemberCache;
  const list = (await ghJson(["api", `/orgs/${org}/members?per_page=100`, "--paginate"])) || [];
  orgMemberCache = list
    .map((m) => m && m.login)
    .filter(Boolean)
    .sort((a, b) => a.localeCompare(b)) // no meaningful recency here, so alphabetical
    .map((login) => ({ handle: login, display: login, isTeam: false }));
  orgMemberAt = Date.now();
  return orgMemberCache;
}
