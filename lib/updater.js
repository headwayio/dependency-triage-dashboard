...
const os = require("os");
const { run, runOrThrow, stream } = require("./exec");
const { assertRepoName, vulnerablePackagesByEcosystem } = require("./github");
...
function readRubyVersion(dir) {
  ...
  return (
    tryFile(".ruby-version", /([0-9]+\.[0-9]+\.[0-9]+)/) || // tolerates leading space
    tryFile(".mise.toml", /^\s*ruby\s*=\s*["']([0-9.]+)["']/m) ||
    tryFile(".tool-versions", /^\s*ruby\s+([0-9.]+)/m) ||
    tryFile("Gemfile", /^\s*ruby\s+["']([0-9]+\.[0-9]+\.[0-9]+)["']/m) // exact literal pins only
  );
}
...
function readNodeVersion(dir) {
  ...
  let v =
    tryFile(".node-version", /v?([0-9]+(?:\.[0-9]+){0,2})/) ||
    tryFile(".nvmrc", /v?([0-9]+(?:\.[0-9]+){0,2})/) ||
    tryFile(".mise.toml", /^\s*node\s*=\s*["']?v?([0-9]+(?:\.[0-9]+){0,2})/m) ||
    tryFile(".tool-versions", /^\s*node(?:js)?\s+v?([0-9]+(?:\.[0-9]+){0,2})/m);
  if (!v) {
    try {
...
function fileMatch(dir, name, re) {
  const p = path.join(dir, name);
  if (!exists(p)) return null;
  const m = fsSync.readFileSync(p, "utf8").match(re);
  return m ? m[1] : null;
...
async function createUpdatePR({ config, repo, emit, upgrade }) {
  ...
  // Stream a child process line-by-line into the SSE log; return exit code.
  const sh = (cmd, args, opts = {}) =>
    stream(cmd, args, { cwd: dir, ...opts }, (line, which) =>
  ...
  await stream(
    "gh",
    ["repo", "clone", nwo, dir, "--", "--depth=1"],
    { cwd: workRoot },
    (line, which) => log(line, which === "stderr" ? "warn" : "info")
  );
