...
// Dependabot's default when dependabot.yml doesn't say otherwise.
const DEFAULT_PR_LIMIT = 5;

/**
 * Open Dependabot PRs on a repo, counted by ecosystem. The branch encodes it —
 * `dependabot/<ecosystem>/<package>-<version>` — using the same labels the update-job
 * titles do, so these keys line up with staleEcosystems()' `label` without translation.
 * Only called for repos already flagged stale, so the cost stays proportional to the
 * problem rather than the org.
 */
async function fetchPrCounts(org, name) {
  const res = await run("gh", [
    "pr", "list", "--repo", `${org}/${name}`, "--state", "open",
    "--author", "app/dependabot", "--json", "headRefName", "--limit", "100",
  ]);
  if (res.code !== 0) return null;
  try {
    const counts = {};
    for (const p of JSON.parse(res.stdout) || []) {
      const eco = String(p.headRefName || "").split("/")[1];
      if (eco) counts[eco] = (counts[eco] || 0) + 1;
    }
    return counts;
  } catch {
    return null;
...
/**
 * Explain a stale ecosystem where we can. Hitting `open-pull-requests-limit` is the cause
 * worth naming: Dependabot stops scheduling that ecosystem entirely once the cap is full,
 * so it looks broken while behaving exactly as configured — and the fix is merging or
 * closing what's already open, which is a different action from anything else here.
 * Counts unavailable (null) leave the entry unannotated rather than guessing at a cause.
 */
function annotateStaleCause(stale, entries, countsByLabel) {
  if (!countsByLabel) return stale || [];
  return (stale || []).map((s) => {
    const entry = (entries || []).find((e) => e.ecosystem === s.ecosystem);
    const limit = (entry && entry.openPrLimit) || DEFAULT_PR_LIMIT;
    const open = countsByLabel[s.label] || 0;
    return { ...s, openPRs: open, prLimit: limit, atLimit: open >= limit };
  });
}
