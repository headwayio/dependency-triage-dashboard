...
/** Today as YYYY-MM-DD in LOCAL time — deliberate: branch names dedupe per local
 *  day, so an evening re-run doesn't open a second same-day branch/PR the way a
 *  UTC stamp would (UTC flips the "day" at ~6-7pm US Central). */
function todayStamp() {
  const d = new Date();
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
}

module.exports = { run, runOrThrow, stream, todayStamp };
