...
// Release a repo's whole fix budget once its CI has settled green. The per-repo total
// exists to stop a fix→new-SHA→fail runaway, and green ends that chain — so a repo the
// fixer SUCCESSFULLY greened N times shouldn't be capped like one where it never worked.
// Without this the budget only ever counts down: a repo that used its attempts well is
// left with nothing for the next, unrelated failure. Returns how many records were cleared.
function clearFixAttempts(repo) {
  const d = fixStore.load();
  let cleared = 0;
  for (const k of Object.keys(d)) if (k.startsWith(`${repo}@`)) { delete d[k]; cleared++; }
  if (cleared) fixStore.save(d);
  return cleared;
}
