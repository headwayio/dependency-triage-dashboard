...
// Oldest first → the bottom of the stack / front of the sequence. Stable: createdAt, then
// PR number, so the order is deterministic even if timestamps tie.
function ordered(prs) {
  return (prs || [])
    .filter((p) => p && p.number && p.headRefName)
    .slice()
...
async function createStackPRs({ config, repo, prs, base, lock, emit }) {
  ...
  await runClaude({ prompt: stackPrompt(nwo, baseBranch, lock || "the lockfile", chainRefs), cwd: dir, log, ...session });
