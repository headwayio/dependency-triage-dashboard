...
const { vulnerablePackagesByEcosystem, fetchInstalledVersions, refreshRepoModel } = require("./github");
const { jobContext } = require("./job");
const { writePins } = require("./eol");
const { parseMixLock } = require("./hex");
...
// Mix evaluates config on EVERY command, and many Elixir repos end `config/<env>.exs` with
// `import_config "<env>.secret.exs"` — a gitignored secrets file that's absent in a fresh clone,
// so every mix command (local.hex, deps.update, …) dies with
// `(File.Error) could not read file ".../<env>.secret.exs"` before doing anything. Before running
// mix, write a minimal stub (`import Config`) for each imported `*.secret.exs` that's missing, so
// config loads. Empty config is fine for `deps.update` (it needs no real secrets), and the stub
// stays gitignored (we only `git add` mix.lock/mix.exs), so it's never committed. Returns the
// stubbed relative paths. Best-effort; only touches files matching the gitignored-secret
// convention, never an arbitrary missing import.
function stubMissingMixSecrets(dir, log) {
  const cfgDir = path.join(dir, "config");
  let entries;
  try { entries = fsSync.readdirSync(cfgDir).filter((f) => f.endsWith(".exs")); } catch { return []; }
  // mix runs in the default env here (we don't set MIX_ENV) → config_env() resolves to "dev".
  const env = process.env.MIX_ENV || "dev";
  const wanted = new Set();
  for (const f of entries) {
    let txt = "";
    try { txt = fsSync.readFileSync(path.join(cfgDir, f), "utf8"); } catch { continue; }
    const re = /import_config\s+"([^"]*\.secret\.exs)"/g;
    let m;
    while ((m = re.exec(txt))) {
      const name = m[1].replace(/#\{\s*config_env\(\)\s*\}/g, env).replace(/#\{\s*Mix\.env\(\)\s*\}/g, env);
      if (name.includes("#{")) continue; // unresolved interpolation we can't safely guess
      wanted.add(name);
    }
  }
  const stubbed = [];
  for (const name of wanted) {
    ...
    try {
      ...
      fsSync.writeFileSync(p, "import Config\n");
      stubbed.push(`config/${name}`);
    } catch { /* best-effort */ }
  }
  if (stubbed.length && log) log(`Stubbed missing secret config(s) so mix can load: ${stubbed.join(", ")} (gitignored; not committed).`, "warn");
  return stubbed;
}
...
// Elixir pin: .tool-versions (asdf-style, often `elixir 1.16.2-otp-26`) / .mise.toml
// / the `elixir: "~> 1.x"` requirement in mix.exs (a floor). The first two are exact,
// mise-installable versions; the mix.exs floor is a weaker best-effort fallback.
function readElixirVersion(dir) {
  return (
    fileMatch(dir, ".tool-versions", /^\s*elixir\s+v?([0-9][0-9.]*(?:-otp-[0-9]+)?)/m) ||
    fileMatch(dir, ".mise.toml", /^\s*elixir\s*=\s*["']?v?([0-9][0-9.]*(?:-otp-[0-9]+)?)/m) ||
    fileMatch(dir, "mix.exs", /elixir:\s*["'][~>=\s]*([0-9]+\.[0-9]+(?:\.[0-9]+)?)/)
  );
}

// Erlang/OTP pin: .tool-versions / .mise.toml. Elixir needs a matching OTP, and mise
// installs the two as separate tools, so we provision both.
function readErlangVersion(dir) {
  return (
    fileMatch(dir, ".tool-versions", /^\s*erlang\s+v?([0-9][0-9.]*)/m) ||
    fileMatch(dir, ".mise.toml", /^\s*erlang\s*=\s*["']?v?([0-9][0-9.]*)/m)
  );
}
...
/**
 * This repo's OPEN tool-opened update PRs (under config.branchPrefix), excluding
 * `keepBranch` (today's branch). Newest first (highest PR number). Used to dedupe across
 * days: a prior day's update PR is still open with a different (date-stamped) branch.
 */
async function findOpenUpdatePRs({ nwo, config, keepBranch }) {
  const prefix = config.branchPrefix || "dependency-updates/soc2";
  const listed = await run("gh", ["pr", "list", "--repo", nwo, "--state", "open", "--json", "number,headRefName,url", "--limit", "100"]);
  if (listed.code !== 0) return [];
  let prs = [];
  try { prs = JSON.parse(listed.stdout || "[]"); } catch { return []; }
  return prs
    .filter((p) => p.headRefName && p.headRefName.startsWith(prefix) && p.headRefName !== keepBranch)
    .sort((a, b) => b.number - a.number);
}

/** Close the given PRs as superseded by `byUrl` (delete their branches). Returns closed URLs. */
async function closePRsAsSuperseded({ nwo, log, prs, byUrl }) {
  const newNum = (byUrl.match(/\/pull\/(\d+)/) || [])[1];
  const comment =
    `Superseded by ${newNum ? "#" + newNum : "a newer update PR"} (${byUrl}) — a fresh dependency-remediation ` +
    `PR for this repo with the current set of changes. Closing this older one to avoid duplicates.`;
  const closed = [];
  for (const pr of prs) {
    const res = await run("gh", ["pr", "close", String(pr.number), "--repo", nwo, "--comment", comment, "--delete-branch"]);
    if (res.code === 0) { log(`Superseded update PR #${pr.number} (${pr.headRefName}) → ${byUrl}`, "warn"); closed.push(pr.url); }
    else log(`Could not close superseded PR #${pr.number}: ${(res.stderr || res.stdout || "").trim()}`, "warn");
  }
  return closed;
}
...
async function createUpdatePR({ config, repo, emit, upgrade }) {
  ...
  const reqElixir = readElixirVersion(dir);
  const reqErlang = readErlangVersion(dir);
  ...
  if (haveMise) {
    ...
    // Erlang before Elixir: an Elixir build/run needs a matching OTP present first.
    if (reqErlang && config.autoInstallElixir !== false) await provision("erlang", reqErlang);
    if (reqElixir && config.autoInstallElixir !== false) await provision("elixir", reqElixir);
  } else {
  ...
  // mix needs BOTH erlang and elixir on PATH — build a multi-tool `mise exec`.
  const miseMixArgs = (() => {
    const tools = [];
    if (reqErlang) tools.push(`erlang@${reqErlang}`);
    if (reqElixir) tools.push(`elixir@${reqElixir}`);
    return tools.length ? ["exec", ...tools, "--"] : ["exec", "--"];
  })();
  const mixCmd = (cmd, args) => toolCmd(miseMixArgs, cmd, args);
  ...
  if (jsLock && has(dir, "package.json") && (byEco["npm"] || []).length) {
    ...
    // Exclude held-back majors (forcedMajors): their patched floor is a DIFFERENT major, so
    // the overrides fallback would force a breaking cross-major bump via resolutions/overrides
    // — defeating minimizeMajorBumps and breaking dependents (this jammed uuid 3→11,
    // @cypress/request 2→3, tough-cookie 2→4, ansi-regex 2→3 into a "same-major" PR and broke
    // Cypress). The majors stay listed in the PR body for their own opt-in PRs.
    const heldMajor = new Set(forcedMajors.map((p) => p.pkg));
    const patchedByPkg = maxPatchedByPkg((repo.packages || []).filter((p) => p.ecosystem === "npm" && !heldMajor.has(p.pkg)));
  ...
  // Hex / Elixir (mix.lock) — these advisories come from our OWN ERLANG scan
  // (lib/hex.js), not the Dependabot feed, which is blind to mix.lock. `mix deps.update`
  // re-resolves the named deps within the mix.exs constraints (the lockfile-only analog
  // of `bundle lock --update`). Runs under the repo's pinned Elixir + Erlang via mise.
  if (has(dir, "mix.lock")) {
    const pkgs = byEco["erlang"] || [];
    if (!pkgs.length) {
      notes.push(`mix.lock present but no targeted Hex advisories mapped — skipped.`);
    } else {
      // Many Elixir repos import a gitignored `config/<env>.secret.exs`; stub any missing one
      // so mix can evaluate config (it does so on every command). Without this, mix dies before
      // deps.update and the failure would be misreported below as a constraint cap.
      const stubbedSecrets = stubMissingMixSecrets(dir, log);
      // Hex/rebar archives must be present and non-interactive for a deps op in a fresh
      // clone; best-effort (a no-op if already installed).
      await mixCmd("mix", ["local.hex", "--force"]);
      await mixCmd("mix", ["local.rebar", "--force"]);
      log(`$ mix deps.update ${pkgs.join(" ")}`);
      const code = await mixCmd("mix", ["deps.update", ...pkgs]);
      if (code === null) {
        notes.push(`mix.lock present but Elixir/mix is unavailable (no mise Elixir/Erlang, not on PATH). Flagged: ${flaggedList("erlang")}.`);
      } else if (code !== 0) {
        notes.push(
          `\`mix deps.update ${pkgs.join(" ")}\` exited ${code} — a patched package is likely capped by a ` +
            `mix.exs constraint (needs a constraint bump), a dep couldn't be fetched, or a required config couldn't load` +
            `${stubbedSecrets.length ? "" : " (e.g. a missing config/*.secret.exs the build imports)"}. ` +
            `Flagged: ${flaggedList("erlang")}. The exact error is in the mix output above.`
        );
      } else {
        // Re-read mix.lock and flag any attempted package still below its patched floor —
        // a mix.exs (or transitive) constraint caps it. Mirrors the Ruby/npm survivor re-audit.
        let lockTxt = "";
        try {
          lockTxt = fsSync.readFileSync(path.join(dir, "mix.lock"), "utf8");
        } catch {
          /* unreadable — leave blockedPkgs unchanged */
        }
        const resolved = parseMixLock(lockTxt);
        const attempted = new Set(pkgs);
        const floors = maxPatchedByPkg((repo.packages || []).filter((p) => p.ecosystem === "erlang"));
        for (const [name, floor] of floors) {
          if (!attempted.has(name)) continue; // held-back majors aren't "blocked" here
          const got = resolved[name] || null;
          if (got && cmpVer(got, floor) >= 0) continue; // reached the floor — fixed
          blockedPkgs.push({
            ecosystem: "erlang",
            pkg: name,
            resolved: got,
            floor,
            reason: got
              ? "Capped below the patched floor by a mix.exs (or transitive) version constraint — raise the blocking requirement (or its parent) to admit the patch."
              : "Not present in mix.lock after the update — likely capped or removed by a constraint; resolve manually.",
          });
        }
        const ex = blockedPkgs.filter((b) => b.ecosystem === "erlang");
        if (ex.length) {
          log(
            `⚠ ${ex.length} flagged Hex package(s) still below their patched floor after \`mix deps.update\` ` +
              `(a constraint caps them): ${ex.map((b) => `${b.pkg} ${b.resolved || "?"}<${b.floor}`).join(", ")}.`,
            "warn"
          );
          notes.push(
            `${ex.length} flagged Hex package(s) couldn't reach their patched floor via \`mix deps.update\` ` +
              `(${ex.map((b) => `${b.pkg} needs ≥${b.floor}`).join(", ")}) — a mix.exs/parent constraint caps them. See the Blocked table below.`
          );
        }
      }
    }
  ...
  if (diff.code === 0) {
    ...
    // Re-fetch this repo's open alerts + tool PRs so the card immediately reflects
    // reality instead of stale pre-merge counts: a merge that resolved the advisories
    // drops them to 0 (and the alert-driven list sheds the repo on the next render),
    // while any still-open advisory stays visible. Best-effort — mutates `repo` in
    // place (so the server's cached model updates too) and rides the done event.
    let refreshed = null;
    if (!upgrade) {
      step("Refreshing advisories");
      try {
        await refreshRepoModel(config, repo);
        refreshed = { counts: repo.counts, ecosystems: repo.ecosystems, packages: repo.packages, openPRs: repo.openPRs, pending: repo.pending };
        log(`Refreshed advisories: ${repo.counts.total} open alert(s)${repo.pending ? "" : ", no open update PR"}.`);
      } catch (e) {
        log(`Couldn't refresh advisories after the no-change run (${e.message}) — counts may be stale until the next Refresh.`, "warn");
      }
    }
    emit("done", { prUrl: null, changed: false, branch, notes, disposition: gemDisposition, blocked: blockedPkgs, closedPRs, nudgedDependabot, refreshed });
    return { prUrl: null, changed: false, notes, disposition: gemDisposition, blocked: blockedPkgs, closedPRs, nudgedDependabot, refreshed };
  }
  ...
  // ---- 5b. Reconcile against an already-open update PR (cross-day dedupe) --
  // The branch is date-stamped, so a prior day's still-open update PR sits on a different
  // branch. If today's freshly-built tree is IDENTICAL to that PR's, there's nothing new —
  // leave it open (don't churn or discard its review work). If it DIFFERS, we open the new
  // PR and supersede the old one(s) below (Copilot will re-flag the same things on the new).
  let priorUpdatePRs = [];
  if (!upgrade) {
    priorUpdatePRs = await findOpenUpdatePRs({ nwo, config, keepBranch: branch });
    if (priorUpdatePRs.length) {
      const prior = priorUpdatePRs[0]; // newest
      // Compare against FETCH_HEAD (the just-fetched tip) — robust even on a shallow clone
      // whose refspec may not maintain an origin/<branch> tracking ref.
      const fetched = await run("git", ["-C", dir, "fetch", "origin", prior.headRefName]);
      const identical = fetched.code === 0 && (await run("git", ["-C", dir, "diff", "--quiet", "HEAD", "FETCH_HEAD"])).code === 0;
      if (identical) {
        log(`An open update PR (#${prior.number}) already has these exact changes — leaving it open; nothing new to push.`, "warn");
        emit("done", { prUrl: prior.url, changed: false, branch: prior.headRefName, notes, disposition: gemDisposition, blocked: blockedPkgs });
        return { prUrl: prior.url, changed: false, notes, disposition: gemDisposition, blocked: blockedPkgs };
      }
    }
  }
  ...
  // Supersede the prior day's still-open update PR(s) now that the fresh one exists, so the
  // repo never accumulates duplicate update PRs. Only when we actually opened/refreshed one,
  // and gated by the same closeObsoletePRs opt-out as the no-change path.
  let closedPRs = [];
  if (!upgrade && prUrl && priorUpdatePRs.length && config.closeObsoletePRs !== false) {
    step("Superseding older update PR(s)");
    closedPRs = await closePRsAsSuperseded({ nwo, log, prs: priorUpdatePRs, byUrl: prUrl });
  }

  emit("done", { prUrl, changed: true, branch, title, notes, disposition: gemDisposition, blocked: blockedPkgs, closedPRs });
  return { prUrl, changed: true, title, notes, disposition: gemDisposition, blocked: blockedPkgs, closedPRs };
}
...
// satisfied on the branch — a no-op (the "—" rows) or a stale Dependabot alert — not work this
// PR remediates. Splitting it out keeps the targeted table honest and free of fake backwards
// "From → floor" arrows. (Lenient compare, matching the rest of the tool; majorRequired stays
// outstanding since its target is a higher major than installed.)
function advisorySatisfied(p) {
  const to = p.target || p.patched;
  return !!(p.installed && to && cmpVer(p.installed, to) >= 0);
}

function prBody(repo, notes, blocked) {
  const all = repo.packages || [];
  blocked = blocked || [];
  const pkgs = all.filter((p) => !advisorySatisfied(p)); // outstanding = actually remediated here
  const satisfied = all.filter(advisorySatisfied);
  const fmtBump = (p) => {
  ...
  // Advisories the branch already satisfies (installed ≥ patched floor). Listed — not silently
  // dropped — because each is either a stale Dependabot alert worth dismissing or, rarely, a
  // still-vulnerable lower transitive copy worth a look. Kept OUT of the targeted table so the
  // count is honest and nothing shows a backwards arrow.
  const satisfiedNote = satisfied.length
    ? [
        ``,
        `### ✓ Already satisfied on \`${repo.defaultBranch || "the default branch"}\` (${satisfied.length})`,
        ``,
        `These flagged advisories already resolve at/above their patched floor here, so this PR ` +
          `doesn't change them — likely **stale Dependabot alerts** worth dismissing (or, rarely, a ` +
          `still-vulnerable lower transitive copy worth verifying).`,
        ``,
        `| Ecosystem | Package | Installed | Patched floor | Advisory |`,
        `| --- | --- | --- | --- | --- |`,
        ...satisfied.map((p) => `| ${p.ecosystem} | \`${p.pkg}\` | ${p.installed || "—"} | ${p.target || p.patched || "?"} | ${p.ghsa || ""} |`),
      ].join("\n")
    : "";
  ...
  return [
    ...
    pkgs.length
      ? `**Bump** flags the change type — **major**/**minor** are worth a closer look; patch is typically non-breaking. ` +
          ...
          `| --- | --- | --- | --- | --- | --- | --- |\n` +
          rows
      : `No outstanding advisories — every flagged advisory is already satisfied on the branch (below). This PR carries only incidental lockfile updates.`,
    majorNote,
    blockedNote,
    satisfiedNote,
    ``,
