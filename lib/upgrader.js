...
async function openClaudePR(ctx) {
  ...
  if (!committed) {
    ...
    return { prUrl: null, changed: false, branch };
  }
  ...
  return { prUrl, changed: true, branch };
}
...
async function createMajorUpgradePRs({ config, repo, packages, emit }) {
  ...
  const { nwo, dir, log, step } = ctx;
  const base = repo.defaultBranch || "main";
  const majors = (packages && packages.length ? packages : (repo.packages || []).filter((p) => p.majorRequired)) || [];
  if (!majors.length) {
    ...
    try {
      const r = await openClaudePR({
        ...
        bodyLines: [
          ...
          ``,
          `This advisory has **no same-major security fix**, so resolving it needs a breaking major upgrade. This PR raises \`${p.pkg}\` and updates this repo's code/tests for the breaking changes.`,
          ``,
          p.ghsa ? `Advisory: ${p.ghsa}` : ``,
          ``,
      ...
      if (r.prUrl) { prUrls.push(r.prUrl); log(`✓ ${p.pkg} → ${to}: ${r.prUrl}`); }
  ...
  const out = { prUrls, changed: prUrls.length > 0, results };
  emit("done", { ...out });
  return out;
}

module.exports = { createUnblockPR, createMajorUpgradePRs, unblockPrompt, majorPrompt };
