...
// Consolidate a stack of dependency-update PRs into ONE "release" PR.
//
// Why this exists: every dep PR in a stack mutates the SAME lockfiles
...
// A REUSED clone (another job created it) can be stale and/or SHALLOW — its knowledge of a
// branch pinned at an old commit, so fetching a force-moved branch (e.g. a CI-fixed PR) into
// it fails and the rollup silently skips that PR. So on reuse, fetch --prune (and unshallow)
// to bring all refs current before any per-PR fetch.
async function ensureClone({ nwo, dir, workRoot, log, onLn }) {
  if (exists(path.join(dir, ".git"))) {
    ...
    // --unshallow fails if it raced to non-shallow; retry a plain prune fetch so refs are current.
...
function rollupPrompt(nwo, base, branch, { merged, deferred, prs }) {
  const lines = [
    `This is \`${nwo}\` — consolidating ${prs.length} dependency-update PRs into ONE release branch \`${branch}\` (cut from \`${base}\`, already checked out here). The point of a rollup is to resolve conflicts and regenerate lockfiles ONCE, in the context of all the work together, so the result is a single coherent PR with every upgrade.`,
    ``,
...
async function createRollupPR({ config, repo, prs, emit }) {
  ...
  if (list.length < 2) {
    log("Need at least two PRs to roll up — nothing to do.", "warn");
  ...
  if (!merged.length && !deferred.length) {
    log("None of the PR branches could be fetched — aborting rollup.", "warn");
    emit("done", { prUrl: null, changed: false });
    return { prUrl: null, changed: false };
  }
  ...
  const sessionCode = await runClaude({ prompt: rollupPrompt(nwo, base, branch, { merged, deferred, prs: list }), cwd: dir, log, ...session });
  ...
  if (staged.code !== 0) {
    await run("git", ["-C", dir, "commit", "-m", `chore(deps): consolidate ${list.length} dependency upgrades`]);
  }
  ...
  // GUARD: never open a release PR (and CLOSE the originals) unless every input PR's commits
  // ACTUALLY landed in the release branch. A failed/auth-broken session can leave a deferred
  // conflict unresolved while a clean merge already advanced HEAD — the head===before check
  // above wouldn't catch it, so we'd open an incomplete PR and close the originals, LOSING the
  // unmerged PR's work.
  //
  // Verify by CONTENT, not by a ref convention: for each PR, take its CURRENT head SHA on
  // origin (ls-remote — authoritative for exactly what closing the PR would discard) and check
  // that commit is an ancestor of the release HEAD. This holds however the session merged it
  // (refs/rollup, branch name, or a hand-resolved merge that keeps the head as a parent), and
  // it catches both an unfetched PR (#421's case) AND a branch that moved on origin mid-run.
  // A merge-base on a commit that isn't present locally errors → counted missing → abort. The
  // failure direction is always safe: leave the PRs open for a retry, never lose work.
  const missing = [];
  for (const pr of list) {
    const ls = await run("git", ["-C", dir, "ls-remote", "origin", pr.headRefName]);
    const sha = (ls.code === 0 && (ls.stdout.trim().split(/\s+/)[0] || "")) || "";
    if (!sha) { missing.push(pr); continue; } // can't resolve origin head → can't confirm it landed
    const anc = await run("git", ["-C", dir, "merge-base", "--is-ancestor", sha, "HEAD"]);
    if (anc.code !== 0) missing.push(pr);
  }
  if (sessionCode !== 0 || missing.length) {
    const why = missing.length
      ? `${missing.map((p) => "#" + p.number).join(", ")} not incorporated (their head commit isn't in the release branch)`
      : `the session exited ${sessionCode}`;
    log(`✗ Rollup aborted — ${why}. Leaving ALL original PRs open and NOT opening a release PR, so no work is lost. If this was an auth error ("401"), re-authenticate the \`claude\` CLI and restart the server, then re-run.`, "warn");
    // Throw away the local release branch so a retry starts clean (nothing was pushed).
    await run("git", ["-C", dir, "checkout", base]).catch(() => {});
    await run("git", ["-C", dir, "branch", "-D", branch]).catch(() => {});
    emit("done", { prUrl: null, changed: false, failed: true });
    return { prUrl: null, changed: false, failed: true };
  }
  ...
  const bodyLines = [
    ...
    ``,
    `Consolidates ${list.length} dependency-update PRs into a single coherent merge: conflicts resolved and lockfiles regenerated once, with every upgrade present together, then the suite run on the combined state.`,
    ``,
    `### Rolled-up PRs`,
  ...
  // DEFENSE-IN-DEPTH: re-verify each PR's current head is in the release branch right before
  // closing it — independent of the guard above — so no refactor or mid-run branch move can
  // ever close a PR whose work didn't actually land. A PR that fails this is left open.
  ...
  for (const pr of list) {
    const ls = await run("git", ["-C", dir, "ls-remote", "origin", pr.headRefName]);
    const sha = (ls.code === 0 && (ls.stdout.trim().split(/\s+/)[0] || "")) || "";
    const contained = sha && (await run("git", ["-C", dir, "merge-base", "--is-ancestor", sha, "HEAD"])).code === 0;
    if (!contained) { log(`⚠ NOT closing PR #${pr.number} — its head isn't in the release branch (left open to avoid losing work).`, "warn"); continue; }
