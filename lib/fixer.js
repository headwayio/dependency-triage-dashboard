...
// Shared context injected (via --append-system-prompt) into EVERY code-editing session the
// dashboard launches — fix, unblock, major-upgrade, rollup, rebase, address-review. It tells
// the session what it's running inside and how to behave; the per-task user prompt carries
// the specific work + the commit/push/PR directives (which differ per task, so they stay out
// of here — e.g. the CI-fix session pushes itself, the others let the dashboard push).
const SESSION_PREAMBLE = [
  `You are running fully autonomously — a headless \`claude -p\` session with NO human in the loop mid-run — inside the Headway Dependency Dashboard. That dashboard is a local tool that triages Dependabot security alerts across the \`headwayio\` GitHub org and launches you to carry out one specific remediation (a dependency update, an unblock, a major upgrade, a release rollup, a rebase, or addressing PR review comments). How to operate here:`,
  ``,
  `- You're working in a FRESH local clone of ONE target repository, on a branch the dashboard already checked out for you. Stay in this repo; don't touch others.`,
  `- NON-INTERACTIVE: never ask questions, request confirmation, or wait for input — nobody can respond. Make the most reasonable decision from the evidence and proceed.`,
  `- A human operator is watching your output stream live in a small log box, so keep narration brief and concrete (what you're doing and why), not verbose.`,
  `- Use the repo's PINNED toolchain through mise so your versions match CI: \`mise exec -- <cmd>\` (run \`mise install\` first if the pinned runtime isn't present). Regenerate lockfiles with the matching package manager rather than editing them by hand.`,
  `- Keep your change MINIMAL and scoped to the task. Don't refactor unrelated code, bump unrelated dependencies, or "improve" things the task didn't ask for.`,
  `- Never \`git add\` tracker/state files that tooling rewrites on its own (\`.beads/\`, \`.DS_Store\`, editor configs) — only the files your task actually changed.`,
  `- Follow the specific task in the user message for what to do and whether to commit / push / open a PR. Unless it says otherwise, the dashboard handles pushing, opening PRs, and replying to review threads — you just make and commit the change.`,
].join("\n");
...
function runClaude({ prompt, mode, cwd, log, timeoutMin, effort, model }) {
  ...
  const args = ["-p", prompt, "--permission-mode", mode, "--output-format", "stream-json", "--verbose", "--append-system-prompt", SESSION_PREAMBLE];
