...
const UNKNOWN = { state: "unknown", headSha: null, isDraft: false, reviewDecision: null, reviewers: [], mergeable: null, mergeStateStatus: null, reviewUnresolved: 0, checks: [], failing: [] };
...
async function fetchPRStatus(nwo, prNumber) {
  const res = await run("gh", [
    ...
    "--json", "statusCheckRollup,headRefOid,isDraft,state,reviewDecision,reviewRequests,mergeable,mergeStateStatus",
  ]);
...
async function fetchPRStatusBatch(prs) {
  ...
  const fragment =
    "fragment prState on PullRequest { isDraft reviewDecision headRefOid mergeable mergeStateStatus " +
