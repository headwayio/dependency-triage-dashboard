...
// Drop majors that already have an open major-upgrade PR — matched by package (not the
// date-stamped branch), so a re-run never opens a duplicate, even on a later day.
function majorsNeedingPR(repo, majors) {
  const open = (repo.openPRs || []).filter((p) => (p.headRefName || "").startsWith("major-upgrade/"));
  return majors.filter((m) => !open.some((p) => p.headRefName.startsWith(`major-upgrade/${slug(m.ecosystem)}-${slug(m.pkg)}-`)));
}
...
async function createMajorUpgradePRs({ config, repo, packages, emit }) {
  ...
  const { nwo, dir, log, step } = ctx;
  const base = repo.defaultBranch || "main";
  const deduped = dedupeMajors(packages && packages.length ? packages : (repo.packages || []).filter((p) => p.majorRequired));
  // Skip any major that already has an open PR (defense in depth — the caller filters too),
  // so re-running never opens duplicates regardless of the date-stamped branch name.
  const majors = majorsNeedingPR(repo, deduped);
  if (!majors.length) {
    log("No major-required advisories left to upgrade — every one already has an open PR.", "warn");
...
module.exports = { createUnblockPR, createMajorUpgradePRs, unblockPrompt, majorPrompt, dedupeMajors, majorsNeedingPR };
