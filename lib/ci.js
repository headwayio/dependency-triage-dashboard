...
async function fetchPRStatusBatch(prs) {
  ...
  prs.forEach((p, i) => {
    ...
    out.set(`${p.nwo}#${p.number}`, {
      ...
      // Unresolved only — NOT also "not outdated". A thread goes outdated when its anchor
      // line changes, which every CI fix, rebase and rollup does; it means the code moved,
      // not that the comment was answered. Filtering them out hid the review button on PRs
      // that genuinely had unaddressed feedback. See countOpenReviewThreads in lib/reviews.js.
      reviewUnresolved: (((node.reviewThreads || {}).nodes) || []).filter((t) => !t.isResolved).length,
      checks,
      failing,
    });
  });
