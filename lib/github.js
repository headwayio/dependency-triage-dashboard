...
async function refreshRepoModel(config, repo) {
  const { org } = config;
  const [alerts, prMap, installed] = await Promise.all([
  ...
  // Rebuild the flagged-package set + counts exactly like buildModel does.
 ...
 * (`gemspec-bump/…`), constraint-unblock PRs (`dependency-unblock/…`), and major
 * upgrades (`major-upgrade/…`) — which carry different titles. Per-repo `gh pr list`
 * so we get `headRefName`, which the org-wide `gh search prs` doesn't expose.
 */
async function fetchToolPRs(org, names, branchPrefix) {
  const prefixes = [branchPrefix || "dependency-updates/soc2", "runtime-upgrade/", "gemspec-bump/", "dependency-unblock/", "major-upgrade/"];
  ...
  await mapLimit(names || [], 8, async (name) => {
    const prs = (await ghJson([
      ...
      "--json", "number,url,isDraft,createdAt,headRefName,title,reviewDecision,reviewRequests",
      "--limit", "50",
    ])) || [];
    for (const pr of prs) {
