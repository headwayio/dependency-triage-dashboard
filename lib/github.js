...
async function enrichComplianceRepos(org, repos) {
  await mapLimit(repos, 16, async (r) => {
    ...
    r.dependsOnOrg = [];
    r.published = null;
    const listing = await run("gh", ["api", `/repos/${org}/${r.name}/contents`]);
    let files = [];
    if (listing.code === 0) {
      try {
        files = (JSON.parse(listing.stdout) || []).map((f) => f.name);
      } catch {
        /* not a JSON array (empty repo / no default branch) */
      }
    }
...
// Every non-archived repo in the org — the full SOC 2 compliance inventory (not just
// repos with open alerts). One paginated call; lightweight fields only.
async function listOrgRepos(org) {
  ...
  if (res.code !== 0) return [];
  try {
    return (JSON.parse(res.stdout) || []).map((r) => ({
      name: r.name,
...
// The archived (read-only) org repos — excluded from the active compliance inventory,
// listed separately so they can be audited / unarchived / deleted.
async function listArchivedOrgRepos(org) {
  const res = await run("gh", [
    "repo", "list", org,
    "--archived", "--limit", "1000",
    "--json", "name,pushedAt,visibility,isPrivate,url",
  ]);
  if (res.code !== 0) return [];
  try {
    return (JSON.parse(res.stdout) || []).map((r) => ({
