...
/**
 * Unresolved review threads on a PR, each reduced to its primary comment. Returns
 * [{ id, isResolved, isOutdated, anchorLost, author, isCopilot, body, path, line, diffHunk,
 * url }]. `id` is the reviewThread node id (used to reply + resolve).
 *
 * Two different notions of "outdated" here, and they disagree:
 *   isOutdated  — GraphQL sets this once ANY newer commit exists, even when the comment
 *                 still maps cleanly onto the current diff. Too eager to show a user.
 *   anchorLost  — the comment's `line` is null, i.e. GitHub can no longer place it in the
 *                 current diff. This is what GitHub's own UI badges as "Outdated".
 * Checked against headwayio/shipwright-ui#19, where all six threads report isOutdated but
 * only the four with a null line carry GitHub's badge.
 */
async function fetchReviewThreads(nwo, number) {
  ...
  const threads = ((nodes && nodes.nodes) || []).map((t) => {
    ...
    return {
      ...
      // Null `line` BEFORE the originalLine fallback below — once that fallback runs, a lost
      // anchor is indistinguishable from a live one.
      anchorLost: c.line == null,
      author: login,
...
// Count of unresolved threads — the cheap signal for the PR row.
//
// Deliberately NOT filtered on isOutdated. A thread goes outdated when the line it was
// anchored to changes, which is something this tool causes constantly: every CI fix, rebase
// and rollup rewrites lines. Outdated means the code MOVED, not that the feedback was
// addressed — GitHub still counts these as unresolved conversations, and still blocks merge
// on them where "require conversation resolution" is on. Excluding them made a PR with real,
// unaddressed Copilot comments show no review button at all, which is the opposite of what
// the count is for. The address and resolve paths below already ignore isOutdated, so this
// makes the count agree with what the console can actually act on.
...
/**
 * Answer a follow-up question about ONE thread's triage verdict.
 *
 * The triage reason is capped at a line, which is right for a list you're scanning and
 * useless when you disagree with it — "likely skip: expected pnpm behavior" gives you no way
 * to check whether that's true. This reopens the same question with room to answer, and
 * keeps the exchange going, so the verdict is arguable rather than just asserted.
 *
 * Stateless: the caller owns the transcript and sends it back each turn. The thread itself is
 * re-read from GitHub rather than trusted from the client, so the model always argues about
 * the comment as it stands now.
 *
 * @param verdict  the prior { recommend, reason }, if triage has run — may be null
 * @param messages [{ role: "user"|"assistant", content }] oldest first, ending with the question
 */
async function askAboutThread(nwo, thread, verdict, messages) {
  const turns = (messages || [])
    .filter((m) => m && typeof m.content === "string" && m.content.trim())
    .slice(-12) // keep the prompt bounded; the thread context matters more than deep history
    .map((m) => `${m.role === "assistant" ? "You" : "Reviewer"}: ${m.content.trim()}`)
    .join("\n\n");
  const prior = verdict
    ? `You previously triaged this comment and recommended "${verdict.recommend}" because: ${verdict.reason || "(no reason given)"}`
    : `This comment has not been triaged yet.`;
  const prompt = [
    `You are helping a maintainer decide what to do about a code-review comment on a dependency-update PR in \`${nwo}\`.`,
    ``,
    prior,
    ``,
    `## The comment`,
    `By: ${thread.author}${thread.isCopilot ? " (GitHub Copilot's automated reviewer)" : ""}`,
    `File: ${thread.path}${thread.line ? `:${thread.line}` : ""}`,
    thread.body,
    thread.diffHunk ? `\nDiff it was written against:\n${thread.diffHunk}` : "",
    ``,
    `## Conversation`,
    turns,
    ``,
    `## How to answer`,
    `- Answer the last question directly. Be concrete and specific — cite the code, the ecosystem's actual behaviour, or the advisory, rather than restating the comment.`,
    `- Two or three short paragraphs at most. No preamble, no headings, no bullet lists unless genuinely enumerating.`,
    `- If the question shows your earlier recommendation was wrong, say so plainly and give the corrected one. Being persuaded is a valid outcome.`,
    `- If you genuinely don't know (you can't see the whole repo), say what you'd need to check rather than guessing.`,
  ].join("\n");
  const text = await runClaudeOneShot({ prompt, timeoutMin: 3, effort: "low" });
