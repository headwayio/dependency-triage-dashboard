...
// Defaults come from settings.js DEFAULTS; the server normally passes
// fully-normalized settings, so the fallbacks only matter for direct callers.
...
function buildClientEmail(repo, installed, opts = {}) {
  const E = { ...DEFAULTS.estimate, ...(opts.estimate || {}) };
  ...
  const cfg = { ...DEFAULTS.email, ...(opts.email || {}) };
